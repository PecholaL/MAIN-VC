...
class CLUBSample_group(nn.Module):  # Sampled version of the CLUB estimator
    ...
    def mi_est(self, x_samples, y_samples):
        x_samples = x_samples.unsqueeze(1).expand(-1, y_samples.shape[1], -1).reshape(-1, x_samples[-1])
...
class MINE(nn.Module):
    ...
    def forward(self, x_samples, y_samples):
        x_samples = x_samples.unsqueeze(1).expand(-1, y_samples.shape[1], -1).reshape(-1, x_samples.shape[-1])
        y_samples = y_samples.reshape(-1, y_samples.shape[-1])
        ...
        lower_bound = numpy.log2(numpy.exp(1)) * (torch.mean(tmp_1) - torch.log(torch.mean(torch.exp(tmp_2))))
