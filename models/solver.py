...
class Solver(object):
    ...
    def build_model(self): 
        ...
        self.mi_club = CLUBSample_group(64,64,64).to('cuda' if torch.cuda.is_available() else 'cpu')
        self.mi_mine = MINE(64,64,64).to('cuda' if torch.cuda.is_available() else 'cpu')
        print('[MAIN-VC]MI net built')
        self.mi_opt = torch.optim.Adam(itertools.chain(self.mi_club.parameters(), self.mi_mine.parameters()), lr=1e-4)
    ...
    def train(self, n_iterations):
        for iteration in range(n_iterations):
            if iteration >= self.config['annealing_iters']:
                ...
                lambda_mi = self.config['lambda']['lambda_mi'] * (iteration + 1) / self.config['annealing_iters']
            data = next(self.train_iter)

            # log-tensorboard
            meta = self.mainvc_step(data, lambda_kl, lambda_mi)
            if iteration % self.args.summary_steps == 0:
                self.logger.add_scalars(f'{self.args.tag}/mainvc_train', meta, iteration)
            # log-terminal
            ...
            print(
                ...
                f'loss_rec={loss_rec:.6f}',    f'loss_kl={loss_kl:.6f}', 
                f'loss_sia={loss_sia:.6f}',    f'loss_mi={loss_mi:.6f}', 
                f'club_loss={club_loss:.6f}'   f'mine_loss={mine_loss:.6f}',   end='\r'
            )
            # autosave
